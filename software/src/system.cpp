...
void CSystem::LoadParamFile(std::string filename, std::vector<std::string> &Distribution, std::vector<std::string> &Names, Eigen::MatrixXd &Matrix){
  std::string line,
    number;
  std::size_t pos;
  std::ifstream myfile(filename);
  ...
  Distribution = std::vector<std::string>(rows);
...
void CSystem::WriteParamFile(std::string fileName, std::vector<std::string> &header, Eigen::MatrixXd &file){
  int rows = file.rows();
  int cols = file.cols();
  std::fstream ofile(fileName,std::ios::out);
  if(ofile.is_open()){
...
void CSystem::WriteParamFileLoop(std::string filename, std::string folder, int start, std::vector<std::string> &header, Eigen::MatrixXd &matrix){
  int rows = matrix.rows();
...
void CSystem::WriteParameterFiles(std::string rangename, std::string foldername, std::string filename, std::string delimiter, int start, int finish, int ab){
  ...
  this->LoadParamFile(rangename,Distribution,Name,range);
  LHCSampling(matrix,finish-start,ab,range);
  this->MkdirLoop(foldername,start,finish);
  this->WriteParamFileLoop(filename,foldername,start,Name,matrix);
}
void CSystem::WritePosteriorParameterFiles(std::string foldername, std::string loadname, std::string paramname, std::vector<std::string> Name, int start, int finish){
  Eigen::MatrixXd matrix;
  
  this->LoadFile(filename,matrix,delimiter);
  this->MkdirLoop(foldername,start,finish);
  this->WriteParamFileLoop(paramname,foldername,start,Name,delimiter,matrix);
}
